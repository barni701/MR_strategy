...
def generate_rsi2_signals(
    ...
    long_entry = above_200 & oversold & (data['VIX'] < 30)
...
def main():
    ...
    VIX_FILEPATH = "Data_1D/VIX_1d.parquet"
    if not os.path.exists(VIX_FILEPATH):
        print(f"VIX data not found at {VIX_FILEPATH}. Please ensure it is available for loading.")
        exit(1)
    
    df = pd.read_parquet(VIX_FILEPATH)
